...
private:
    ...
    HTTP_CODE parse_request_line(char *text); // 请求首行
    HTTP_CODE parse_headers(char *text); // 请求头
    ...
    void unmap(); // 释放内存映射
    bool add_response(const char *format, ...); // 添加响应行
    bool add_content(const char *content);  //响应体
    bool add_content_type(); // 响应类型
    bool add_status_line(int status, const char *title); // 状态码
...
private:
    ...
    struct iovec m_iv[2];                // 将采用writev来执行写操作，定义下面两个成员，其中m_iv_count表示被写内存块的数量。
    int m_iv_count;
